...
async function handleClientSub(request, env, subId, url) {
  ...
  if (!rawData) {
    return new Response(JSON.stringify({ error: "未找到订阅 ID: " + subId }, null, 2), {
      status: 404,
      headers: { "Content-Type": "application/json; charset=utf-8" }
    });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "订阅元数据损坏" }), { status: 500 });
  }
  ...
  return convertFromUrl(subInfo.sourceUrl, { targetVersion, enableTun, rejectRules });
}
...
function parseClashYaml(yamlStr) {
  ...
  const pushProxy = (p) => {
    if (p && Object.keys(p).length > 0) {
      const conv = convertClashItem(p);
      if (conv) nodes.push(conv);
    }
  };

  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith("proxies:")) { inProxies = true; continue; }
    // New top-level section — leave proxies
    if (inProxies && /^[a-zA-Z0-9_-]+:/.test(trimmed) && !trimmed.startsWith("-")) break;
    if (!inProxies) continue;

    // Inline format: - {name: xxx, ...}
    const inlineMatch = trimmed.match(/^-\s*\{([\s\S]*)\}\s*$/);
    if (inlineMatch) {
      pushProxy(currentProxy);
      currentProxy = null;
      pushProxy(clashParseInlineMap(inlineMatch[1]));
      continue;
    }

    // Block format: - or   key: value
    if (trimmed.startsWith("-")) {
      pushProxy(currentProxy);
      currentProxy = {};
      const rest = trimmed.substring(1).trim();
      if (rest && rest.includes(":")) clashSetKV(rest, currentProxy);
    } else if (currentProxy && trimmed.includes(":")) {
      clashSetKV(trimmed, currentProxy);
    }
  }
  pushProxy(currentProxy);
  return nodes;
...
function convertClashItem(c) {
  ...
  function buildTransport() {
    ...
    if (net === "ws" || net === "websocket") {
      const wo = c["ws-opts"] || {};
      const wh = (wo && wo.headers) || {};
      return {
        type: "ws",
        path: wo.path || "/",
        headers: { Host: wh.Host || wh.host || c.servername || server }
      };
    }
    if (net === "grpc") {
